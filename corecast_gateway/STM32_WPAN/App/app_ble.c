        ...
        case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          /* USER CODE BEGIN HCI_EVT_LE_ADVERTISING_REPORT */
          {
              ...
              for (int i = 0; i < num_reports; i++) {
                  ...
                  // 2. 우리 기기라면 데이터 파싱 진행
                  if (found_idx != -1) {
                	  printf("[BLE_RX] dev=%d data_len=%d tick=%lu\r\n", found_idx, data_len, HAL_GetTick());
                	  uint8_t j = 0;
                      ...
                      while (j < data_len) {
                          ...
                          // field_type이 0xFF(Manufacturer Data), 최소 길이 14 (구 포맷) 이상이면 처리
                          // 구 포맷: type1B + CID2B + temp1/temp2/x/y/z 각2B + reason1B          = 14B
                          // 신 포맷: type1B + CID2B + temp1/temp2/x/y/z 각2B + ESP_ID1B + core_temp2B = 16B
                          // → 신/구 둘 다 수용. core_temp는 길이가 될 때만 읽는다.
                          if (field_type == 0xFF && field_len >= 14) {
                              ...
                              if (cid_low == 0x34 && cid_high == 0x12) {
                                  if (HAL_GetTick() - myDevices[found_idx].last_tick >= 900) {
                                      ...
                                      /* 신 포맷일 때만: adv_data[j+14]=ESP_DEVICE_ID(미사용), adv_data[j+15..16]=core_temp */
                                      if (field_len >= 16)
                                          data.core_temp_x100 = (int16_t)(adv_data[j+15] | ((uint16_t)adv_data[j+16] << 8));
                                      else
                                          data.core_temp_x100 = 0;
